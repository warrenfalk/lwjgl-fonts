...
/**
 * FTGlyph is the base class for FTGL glyphs. It provides the interface between
 * Freetype glyphs and their openGL renderable counterparts. This is an abstract
 * class and derived classes must implement the <code>render</code> function.
 * 
...
public abstract class FTGlyph {
	 ...
	 * 
	 * @param glyph
	 *            The Shape to put in the FTGlyph.
	 */
	public FTGlyph(Shape glyph) {
		if (glyph != null) {
			bBox = new FTBBox(glyph);
			this.glyph = glyph;
			// several options:
			/** due to {@link ShapeGraphicAttribute#getAdvance()} description */
			advance = Math.abs(bBox.upperX - bBox.lowerX);
			/** due to {@link ShapeGraphicAttribute} */
			float shapeadvance = new ShapeGraphicAttribute(glyph,
					GraphicAttribute.CENTER_BASELINE,
					ShapeGraphicAttribute.STROKE).getAdvance();
			/**
			 * or create a {@link java.awt.font.TextLayout} and get the advance
			 * for the letter, the shape represents
			 */
			// System.out.println("[Boxed] " + this.advance +
			// " [ShapeAttribute] " + shapeadvance);
			advance = shapeadvance;
		}
	}
	 ...
	 * 
	 * @param x
	 *            The current pen position's x component.
	 * @param y
	 *            The current pen position's y component.
	 * @param z
	 *            The current pen position's z z component.
