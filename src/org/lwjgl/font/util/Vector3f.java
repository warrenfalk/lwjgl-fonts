...
public class Vector3f implements Cloneable {
	...
	// public float dx, dy; // tkoch: only for test purposes regarding inclusion
	// of .3DS loader
	 ...
	 * @return <code>true</code> if this vector has the same components as the
	 *         argument vector <code>v</code>, false otherwise
	 */
	 ...
	 * Compares this vector with the given vector.
	 * <p>
	 * Note: tolerance is applied per component, thus all vectors which equal
	 * this vector will form a box.
	 * </p>
	 * 
	 * @param v
	 *            the vector to be used for compare
	 * @param tolerance
	 *            accepted tolerance
	 * @return <code>true</code> if this the components of this vector are equal
	 *         to the argument vector <code>v</code> with a tolerance of
	 *         argument <code>tolerance</code>, false otherwise
	 */
	public boolean equals(Vector3f v, float tolerance) {
		if (v == null)
			return false;

		return (Math.abs(x - v.x) < tolerance && Math.abs(y - v.y) < tolerance && Math
				.abs(z - v.z) < tolerance);
	}
	...
	@Override
	public int hashCode() {
		int i1 = Float.floatToIntBits(x);
		int i2 = Float.floatToIntBits(y);
		int i3 = Float.floatToIntBits(z);
	...
	/**
	 * Improved version of {@link #normalize()} which also works for VERY short
	 * vectors, or very large ones. This implementation is slighty slower than
	 * {@link #normalize()}.
	 * 
	 * @return this
	 */
	public Vector3f normalize2() {
		...
		while (x > -0.5f && x < 0.5f && y > -0.5f && y < 0.5f && z > -0.5f
				&& z < 0.5f) {
			x *= 2;
			y *= 2;
			z *= 2;
		...
		while (x <= -1.0f || x >= 1.0f || y <= -1.0f || y >= 1.0f || z <= -1.0f
				|| z >= 1.0f) {
			x /= 2;
			y /= 2;
			z /= 2;
	...
	public boolean isInvalid() {
		return Float.isInfinite(x) || Float.isInfinite(y)
				|| Float.isInfinite(z) || Float.isNaN(x) || Float.isNaN(y)
				|| Float.isNaN(z);
	}
	...
	public float angle(final Vector3f v) {
		float dls = dot(v) / (length() * v.length());
		if (dls < -1.0f)
			dls = -1.0f;
		else if (dls > 1.0f)
			dls = 1.0f;
	...
	public Vector3f cross(final Vector3f v) {
		final float a = y * v.z - z * v.y;
		final float b = z * v.x - x * v.z;
		final float c = x * v.y - y * v.x;
	...
	/**
	 * Returns <code>true</code> if this vector is normalized. If it's length is
	 * <code>1</code>.
	 * 
	...
	/**
	 * Returns <code>true</code> if this vector is normalized. If it's length is
	 * <code>1</code> within a tolerance of argument <code>tolerance</code>.
	 * 
	 * @param tolerance
	 *            amount of error to be ignores - must be positive!
	 *            <p>
	 *            Note: the normal is computed by using {@link #lengthSquared()}
	 *            , thus the following equation approximated. For a Vector3f
	 *            <code>v</code>: Correct:
	 * 
	 *            <pre>
	 ...
	 * </pre>
	 * 
	 *            This test is infact less precise than the original test, but
	 *            seems to be reasonable, as a tolerance is assumed to be pretty
	 *            small!
	 * @return <code>true</code> if this vector is normalized.
	 */
	public boolean isNormalized(float tolerance) {
		assert tolerance >= 0 : "tolerance must be positive or 0";
		float tmp1 = lengthSquared() - 1f;
		float tmp2 = 2f * tolerance + tolerance * tolerance;
		return tmp1 < tmp2 && -tmp1 < tmp2;
	}

	/**
	 * Returns <code>true</code> if this vector is orthogonal to argument
	 * <code>other</code>.
	 * 
	 * @param other
	 *            other vector
	 * @return <code>true</code> if this vector is orthogonal to argument
	 *         <code>other</code>.
	 */
